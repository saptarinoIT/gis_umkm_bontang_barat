...
<form action="{{ route('umkm.data.update', $umkm->id) }}" method="POST" enctype="multipart/form-data">
  @csrf
  @method('PATCH')
  <div class="form-group mb-3">
    <label for="">User UMKM</label>
    <input type="hidden" name="user_id" value="{{ $userId }}">
    <input type="text" class="form-control" id="" readonly value="{{ $userId }} - {{ ucwords($userName) }}">
    @error('user_id')
